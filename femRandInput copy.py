...
class randRhoFilterGen:
    ...
    def genNext(self)->None:
        ...
        filt = self.filts[self.iPass][self.iSkew][self.iDir]
        self.data = randBulkUni(self.nx,self.ny,Fxmax=1.0, win=filt)
...
class randBoundaryFilter1DGen:
    def __init__(self,nx=64,ny=64,passes = [16,32,48,64]) -> None:
        self.nx = nx
        self.ny = ny
        self.nb = nx*2+ny*2
        self.passes = passes
    ...
    def genNext(self)->None:
        self.iPass = rng.integers(0,len(self.passes))
        filt = self.filts[self.iPass]
        self.data = randLineUni(self.nb,Fxmax=1.0, win=filt)
        pass
